...
beforeAll(async () => {
  // Now load the modules AFTER the mock is in place
...
test("getBookLists returns 200 and JSON", async () => {
  const mockCursor = { toArray: jest.fn().mockResolvedValue([{ id: 1 }]) };

  BookListService.getBookLists.mockResolvedValue(mockCursor);
  ...
  expect(BookListService.getBookLists).toHaveBeenCalled();
